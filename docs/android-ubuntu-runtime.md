...
- `execute_ubuntu_command` runs GNU/Linux commands in an app-private Ubuntu
  24.04 ARM64 root filesystem through PRoot. It always retains kcode's Android
  application UID; PRoot's `-0` option only emulates Linux root metadata.
...
- Extraction occurs under `filesDir/ubuntu_runtime/installing`; every archive
  path is containment-checked, symlink traversal is rejected, device nodes are
  skipped because `/dev` is bound at runtime, and the completed root is moved
...
- The host process environment is cleared. `PROOT_LOADER` and `PROOT_TMP_DIR`
  ...
  `PATH`.
- `/workspace` is bound directly to the same private agent workspace used by
  kcode file tools. Readable `/dev`, `/proc`, `/sys`, and the current Android
  user's emulated storage are added when available.
...
The runtime is intentionally not a VM: it shares the Android kernel and kcode's
UID. `apt`, Python, Node.js installed through apt, compilers, and ordinary Linux
CLI software work, while systemd, kernel modules, real mounts, and privileged
kernel operations do not.
